...
static int prepend_rebindings(struct rebindings_entry **rebindings_head,
                              struct rebinding rebindings[],
                              size_t nel) {
    struct rebindings_entry *new_entry = (rebindings_entry *)malloc(sizeof(struct rebindings_entry));
    if (!new_entry) {
        return -1;
    }
    new_entry->rebindings = (rebinding *)malloc(sizeof(struct rebinding) * nel);
...
static inline thread_call_stack * get_thread_call_stack() {
    ...
    if (cs == NULL) {
        ...
        cs->stack = (thread_call_record *)malloc(128 * sizeof(thread_call_record));
...
static void hook_Objc_msgSend() {
    ...
    /*
     objc_msgSend(self, @selector(xxxxxx), argus...)
     x0 里面存放self的地址
     x1 selector的地址
     可能一直到x7 都存放着参数
     所以先将这部分寄存器放入内存中，做好现场保护
     系统的syscall函数会将number放到x8里面
     (msgSend利用x0 - x8的寄存器存储参数信息)
     这里没有将lr存入栈空间
    ...
    //lr = 0x0000000104bc18cc
    // Call through to the original objc_msgSend.
    call(blr, orig_objc_msgSend)
    
    //lr = 0x0000000104a4d8f8
    // Save original objc_msgSend return value.
    save()
    
    //lr = 0x0000000104bc18f8
    // Call our after_objc_msgSend.
    call(blr, &after_objc_msgSend)
    
    // restore lr
    __asm volatile ("mov lr, x0\n");
    //lr = 0x0000000105104364
